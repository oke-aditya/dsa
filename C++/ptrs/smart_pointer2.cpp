...
// way that no two pointers should contain the same object. Assignment transfers ownership
// and resets the rvalue auto pointer to a null
// pointer. Thus, they can’t be used within STL containers due to the aforementioned inability to be copied.
...
// It explicitly prevents copying of its contained pointer
// as would happen with normal assignment i.e. it allows exactly one owner of the underlying pointer.
// there can only be at most one unique_ptr at
// any one resource and when that unique_ptr is destroyed, the resource is automatically claimed
...
// So, it’s best to use unique_ptr when we want a
// single pointer to an object that will be reclaimed when that single pointer is destroyed.
