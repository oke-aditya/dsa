...
// Variadic function templates are functions which can take multiple number of
// arguments.
...
// depend on the input types (e.g. adding two ints should probably return an
// int).
...
// ‘recursive’ case, accepting one or more arguments T and Rest. (Recall that a
// template parameter pack can be empty!)

// How exactly does this work? Let’s trace what happens when
// we try to call, for example, sum(1.0, 2.0, 3.0).
// This is going to be a bit repetitive, but it’s worth it to walk through the
...
// the base case does not apply (it only accepts a single argument), so we
// select the recursive case.
...
// the template argument – this has the effect of transforming sum(rest...) to
// sum(2.0, 3.0).
...
// case. The base case once again does not apply as we have more than one
// argument, so we select the recursive case.

// Type deduction is performed – the compiler deduces T = double,
// and Rest = <double>. It’s subtle, but notice that we have now
// unpacked our original <double, double> pack to T = double and Rest =
// <double>.
