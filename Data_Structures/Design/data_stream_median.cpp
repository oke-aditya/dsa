...
//     data structure. double findMedian() returns the median of all elements so
...
// We can maintain an integer array of length 100 to store the count of each
// number along with a total count. Then, we can iterate over the array to find
// the middle value to get our median.
...
// The left heap is containing elements in decreasing order (Max to Min) and
// right heap is
//  in increasing order (Min to Max) but all the elements are greater than the
//  left heap elements. We start pushing the elements in left heap and try to
//  balance it everytime by comparing with the new elements and with heap size
//  and accordingly put it into the right heap.
