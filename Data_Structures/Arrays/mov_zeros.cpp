...
void swap_zeros_simple(int arr[], int n) {
  ...
  // While traversing, maintain count of non-zero elements in array. Let the
  // count be ‘count’. For every non-zero element arr[i], put the element at
  // ‘arr[count]’ and increment ‘count’. After complete traversal, all non-zero
  // elements have already been shifted to front end and ‘count’ is set as index
  // of first 0. Now all we need to do is that run a loop which makes all
