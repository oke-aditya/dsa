...
// from the root to the left-most node ie- the leaf node you could reach when
// you always travel preferring the left subtree over the right subtree. Leaf
// nodes: All the leaf nodes except for the ones that are part of left or right
// boundary. Reverse right boundary nodes: defined as the path from the
...
// over the left subtree. Exclude the root from this as it was already included
// in the traversal of left boundary nodes.
...
// 2. Print all leaf nodes from left to right, which can again be sub-divided
// into two sub-parts: …..2.1 Print all leaf nodes of left sub-tree from left to
// right. …..2.2 Print all leaf nodes of right subtree from left to right.
