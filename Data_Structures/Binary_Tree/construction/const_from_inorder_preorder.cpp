...
// we will get the position where left subtree ends. And that's where right
// subtree solves. we can call this recursively.
...
Node *solve(vector<int> &preorder, int preStart, int preEnd,
  ...
  root->left = solve(preorder, preStart + 1, preStart + numLeft, inorder,
  ...
  root->right = solve(preorder, preStart + numLeft + 1, preEnd, inorder,
...
Node *buildTree(vector<int> &preorder, vector<int> &inorder) {
  ...
  Node *root = solve(preorder, 0, preorder.size() - 1, inorder, 0,
