...
// Solution: -
// Nodes at level n can be 2 ^ n. So this means we need to handle integer
// overflow. Even 20 nodes can make 2 ^ 20 = 10 ^ 6. We need to do a simple BFS.
// and find number of nodes at the level.
...
// The index of the left and right child nodes are 2*parent_index + 1 and
// 2*parent_index + 2 respectively. Thus, the space and time required to process
// the
...
int widthOfBinaryTree(Node *root) {
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (node->left) q.push({node->left, 2 * right_index + 1});

      if (node->right) q.push({node->right, 2 * right_index + 2});
    }
