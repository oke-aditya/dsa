...
// But if we do this we will end up doing DFS on every cell which will take O(m * n) ^ 2 time
// Hence we play a trick here
// the first row & column is close to the pacific ocean, we use that to find if they can reach atlantic ocean
// the last column & row is closer to atlantic ocean, we use that to find if they can reach pacific ocean.'
// do 2 dfs, one on pacific ocean, one on atlantic ocean and then find intersection between the visited
// 
// 
...
class Solution {
    ...
    void dfs(int x, int y, vector<vector<int>> &grid, vector<vector<int>> &visited, int m, int n)
    {
        ...
        for(int i=0; i<4; i++)
        {
            ...
            if(new_x >= 0 && new_x < m && new_y >= 0 && new_y < n && !visited[new_x][new_y] && grid[new_x][new_y] >= grid[x][y])
            {
