...
//     pixels
//      and modifying their color if it matches the original color of the
//      starting pixel.
...
// If the initialColor does not match the color that we are gonna color, then
// don't that's the condition in the question else do the same flood fill trick
...
class Solution {
 public:
  ...
  void dfs(vector<vector<int>>& image, int x, int y, int color, int m, int n,
           int initialColor) {
    image[x][y] = color;
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (new_x >= 0 && new_x < m && new_y >= 0 && new_y < n &&
          image[new_x][new_y] == initialColor) {
...
class Solution2 {
 public:
  ...
  // pass by reference to do inplace
  void bfs(vector<vector<int>>& image, int x, int y, int color, int m, int n,
           int initialColor) {
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (new_x >= 0 && new_x < m && new_y >= 0 && new_y < n &&
            initialColor == image[new_x][new_y]) {
          q.push({new_x, new_y});
