...
// Here too we abuse visited to do both the jobs, distance as well as the visited tracking
// For all other nodes adjacent to 1, do a BFS
// visited on every new neighbor will be 1 + visited of previous x and y
// the maximum value is the farthest value in the island
            ...
            while(!q.empty())
            {
                ...
                while(l--)
                {
                    ...
                    for(int i=0; i<4; i++)
                    {
                        ...
                        if(new_x >= 0 && new_x < m && new_y >= 0 && new_y < n && visited[new_x][new_y] == -1 && grid[new_x][new_y] == 0)
                        {
