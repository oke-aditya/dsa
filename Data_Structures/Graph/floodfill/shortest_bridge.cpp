...
class Solution 
{
    public:
        ...
        void dfs(int x, int y, vector<vector<int>>& grid, vector<vector<int>>& visited, int m, int n, queue<pair<int, int>>& q) 
        {
            visited[x][y] = 1;
            q.push({x, y});
            for (int i = 0; i < 4; i++) 
        ...
        int shortestBridge(vector<vector<int>>& grid) 
        {
            ...
            // Step 1: Use DFS to find the first island and add its boundary cells to the queue
            for (int i = 0; i < m; i++) 
            {
                for (int j = 0; j < n; j++) 
                {
            ...
            {
                int len = q.size();
                min_dist += 1;  // Each level of BFS represents one step away from the first island
                while (len--) 
                {
                    ...
                    for (int i = 0; i < 4; i++) 
                    {
                        ...
                        if (xd >= 0 && yd >= 0 && xd < m && yd < n) 
                        {
                            if (!visited[xd][yd]) 
                            {
                                if (grid[xd][yd] == 1) 
                                {
                                    return min_dist - 1;  // If we encounter the second island, return the distance
                                } 
            ...
            return -1;  // In case there is no path (although it's guaranteed to find one in this problem)
        }
};
