...
// grid are considered water cells. An island in grid2 is considered a
...
// making an new result matrix to store every dfs iteration will lot more space
// for MLE We should check grid2 and grid1, and make use of these 2 in the dfs
// call On every dfs call check from grid2 and grid1 and mark flag as false
...
class Solution {
 public:
  ...
  void dfs(int x, int y, vector<vector<int>>& visited,
           vector<vector<int>>& grid2, vector<vector<int>>& grid1, int m, int n,
           bool& flag) {
    visited[x][y] = 1;

    // Check if this cell in grid2 is part of an island and it also exists in
    // grid1
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (new_x >= 0 && new_x < m && new_y >= 0 && new_y < n &&
          !visited[new_x][new_y] && grid2[new_x][new_y] == 1) {
  ...
  int countSubIslands(vector<vector<int>>& grid1, vector<vector<int>>& grid2) {
    ...
    vector<vector<int>> visited(
        m, vector<int>(n, 0));  // Track visited cells in grid2
    int count = 0;

    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        // Start DFS when we find a cell in grid2 that is part of an island (1)
