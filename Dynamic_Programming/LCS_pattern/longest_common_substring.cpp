...
int lcsub_rec(string s1, string s2, int m, int n, int count)
{
    ...
    else
    {
        ...
        {
            count = max(count, max(lcsub_rec(s1, s2, m-1, n, 0), lcsub_rec(s1, s2, m, n-1, 0)));
        }
        return count;
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    cout<<"Length of Longest Common Substring is " << lcsub_bu(s1, s2, m, n)<<endl;
    // cout<<"Length of Longest Common Substring is " << lcsub_mem(s1, s2, m, n)<<endl;
