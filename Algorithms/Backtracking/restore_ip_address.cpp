...
//  "0.011.255.245", "192.168.1.312" and
...
vector<string> restoreIpAddresses(string s) {
  ...
  for (int i = 1; i < n - 2 && i < 4; i++) {  // First segment (1-3 characters)
    for (int j = i + 1; j < n - 1 && j < i + 4;
         j++) {  // Second segment (1-3 characters)
      for (int k = j + 1; k < n && k < j + 4;
           k++) {  // Third segment (1-3 characters)
        ...
        if (is_valid_ip(s1) && is_valid_ip(s2) && is_valid_ip(s3) &&
            is_valid_ip(s4)) {
