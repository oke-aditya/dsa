...
// The rat can move in four possible directions: 'U'(up), 'D'(down), 'L' (left),
// 'R' (right).
...
// cell is 0, the rat cannot move to any other cell. In case of no path, return
// an empty list.

// The task is to find all possible paths the rat can take to reach the
// destination, starting from (0, 0) and ending at (n-1, n-1), under the
// condition that the rat cannot revisit any cell along the same path.
...
bool is_solved(int x, int y, int n) { return ((x == n - 1) && (y == n - 1)); }
...
void backtrack(vector<vector<int>> mat, int x, int y, int n,
               vector<Choice> &choices, string &sol, vector<string> &res) {
  if (is_solved(x, y, n)) {
    res.push_back(sol);
    return;
  }
