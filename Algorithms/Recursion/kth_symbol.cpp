...
// similarly, when a node is 1, two children nodes are 1 and 0. We can know whether the position of K is a
// left node or a right node by dividing 2. If K is even, current node is right child,
// and its parent is the (K/2)th node in previous row; else if K is odd, current node is left
...
// we still cannot determine current node value. That's why we need recursion,
