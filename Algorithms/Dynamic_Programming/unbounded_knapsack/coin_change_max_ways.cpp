...
int min_coin_rec(int coins[], int n, int change) {
  ...
  if (change == 0) {
    ...
    if (coins[n - 1] > change) {
      ...
      return (min_coin_rec(coins, n, change - coins[n - 1]) +
              min_coin_rec(coins, n - 1, change));
    }
  }
}
...
int min_coin_mem(int coins[], int n, int change) {
  ...
  if (dp[n][change] != -1) {
    ...
    if (coins[n - 1] > change) {
      ...
      dp[n][change] = (min_coin_mem(coins, n, change - coins[n - 1]) +
                       min_coin_mem(coins, n - 1, change));
    }
  }
  return dp[n][change];
}
