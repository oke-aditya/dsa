...
// or equal to W. You cannot break an item, You can pick same item multiple
// times
...
int unboundedKnapsack_rec(int val[], int wt[], int w, int n) {
  if (n == 0 || w == 0) {
    ...
    if (wt[n - 1] > w) {
      ...
      return (max(val[n - 1] + unboundedKnapsack_rec(val, wt, w - wt[n - 1], n),
                  unboundedKnapsack_rec(val, wt, w, n - 1)));
    }
  }
}
...
int unboundedKnapsack_mem(int val[], int wt[], int w, int n) {
  ...
  if (dp[w][n] != -1) {
    ...
    if (wt[n - 1] > w) {
      ...
      dp[w][n] =
          max(val[n - 1] + unboundedKnapsack_mem(val, wt, w - wt[n - 1], n),
              unboundedKnapsack_mem(val, wt, w, n - 1));
      return dp[w][n];
    }
  }
}
