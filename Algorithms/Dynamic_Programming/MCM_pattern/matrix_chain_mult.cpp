...
// mcm(arr, i, k) + mcm(arr, k+1, j) and solve the middle sub problem arr[i-1] * arr[k] * arr[j]
// Computer minimum at each step and return the overall minimum solution.
...
int mcm_rec(int arr[], int i, int j)
{
    ...
    for(k = i; k<j; k++)
    {
        count = mcm_rec(arr, i, k) + mcm_rec(arr, k+1, j) + arr[i-1] * arr[k] * arr[j];
    }
...
int main(int argc, char const *argv[])
{
    ...
    cout << "Minimum number of multiplications is " << mcm_rec(arr, 1, n - 1) <<endl;

    memset(dp, -1, sizeof(dp));

    cout << "Minimum number of multiplications is " << mcm_mem(arr, 1, n - 1) <<endl;
