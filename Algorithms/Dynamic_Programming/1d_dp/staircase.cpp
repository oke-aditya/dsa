...
// There are n stairs in total, x can either climb 1, 2 or 3 stairs in a single move. 
// In how many ways can x reach the last stair i.e. the nth stair.
// Input : 4
// Output : 7
...
int staircase_mem(int n)
{
    ...
    else
    {
        ...
        {
            dp[n] = (staircase_mem(n -1) + staircase_mem(n - 2) + staircase_mem(n - 3));
            return dp[n];
        }
    }
}
