...
// The task is to remove or delete the minimum number of characters from the string so
...
// The remaining characters have to be deleted to make the resultant string to a palindrome.
// minimum number of deletions = n - len of longest palindromic substring.
...
int pal_subs_mem(string s1, int m, string s2, int n)
{
    ...
    else
    {
        ...
        {
            dp[m][n] = (max(pal_subs_mem(s1, m-1, s2, n), pal_subs_mem(s1, m, s2, n-1)));
        }
        return dp[m][n];
    }
}
